...
use winapi::um::winreg::{RegOpenKeyExW, RegQueryValueExW, RegSetValueExW, HKEY_CURRENT_USER};
...
pub fn get_registry_value(
    ...
    let hkey_handle = open_registry_key(registry_key_name, KEY_READ)?;
    ...
    if value_type_res > 0 {
        return Err(RegeditError::new("Failed to read registry value type."));
    }
    ...
    match read_res {
        ...
        _ => Err(RegeditError::new("Failed to read registry value.")),
    }
}
...
fn set_registry_value(
    registry_key: &str,
    registry_value: &str,
    registry_value_type: &RegistryValue,
    mut data: Vec<u8>,
) -> Result<()> {
    let hkey_handle = open_registry_key(registry_key, KEY_SET_VALUE)?;

    let lp_value = registry_value.to_lpcwstr();
    ...
    match set_res {
        0 => Ok(()),
        _ => Err(RegeditError::new("Failed to set registry value.")),
    }
}
...
fn open_registry_key(
    ...
    let lp_sub_key = registry_key.to_lpcwstr();
    ...
    match open_key_res {
        0 => Ok(hkey_handle),
        _ => Err(RegeditError::new("Failed to open registry key.")),
    }
}
